...
%% Manual Cropping
% Restricting our analysis to smaller volume
CD = load_dat(fullfile(directory,'CD.dat'),[matrix(1) matrix(2) matrix(3)]);
mip1 = squeeze(max(CD,[],1));
figure; imshow(mip1,[]); title('DRAW RECTANGLE FOR CROPPING DIMS 2 and 3');
rect1 = drawrectangle; %draw a rectangular field of view
pos1 = round(rect1.Position); %get positions of rectangle edges
IDXstart(2) = pos1(2);
IDXend(2) = pos1(2)+pos1(4);
IDXstart(3) = pos1(1);
IDXend(3) = pos1(1)+pos1(3);

CD2 = CD(:,IDXstart(2):IDXend(2),IDXstart(3):IDXend(3));
mip2 = squeeze(max(CD2,[],2));
figure; imshow(mip2,[]);
rect2 = drawrectangle; title('DRAW RECTANGLE FOR CROPPING DIM 1');
pos2 = round(rect2.Position); %get positions of rectangle edges
IDXstart(1) = pos2(2);
IDXend(1) = pos2(2)+pos2(4);

% Crop data with new dimensions
MAG = MAG(IDXstart(1):IDXend(1),IDXstart(2):IDXend(2),IDXstart(3):IDXend(3));
newDIM = size(MAG);
clear CD* mip* rect* pos1 pos2
...
SMf = 10;
shiftHM_flag = 0; %flag to shift max curvature by FWHM
