function [area_val,diam_val,flowPerHeartCycle_val,maxVel_val,PI_val,RI_val,flowPulsatile_val,...
    ...
    BGPCdone,directory,nframes,res,MAG,IDXstart,IDXend,handles,varargin)
...
%% Load in rec data if PROUD
if strcmp(filetype,'rec')
    % no loading necessary, grab from varargin
    vxx = varargin{1};
    vyy = varargin{2};
    vzz = varargin{3};
end
...
for j = 1:nframes
    ...
    elseif strcmp(filetype,'rec')   % no loading necessary, already loaded
        vx = vxx(:,:,:,j);
        vy = vyy(:,:,:,j);
        vz = vzz(:,:,:,j);
