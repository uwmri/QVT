...
for j = 1:nframes
    if strcmp(filetype,'dat')
        disp('Processing of dat files is not supported in this branch because spatial coordinates have been reversed for consistency with velocity')
        return
    ...
    elseif strcmp(filetype,'hdf5')
        ...
        vx = single(wraphdf5reader(fullfile(directory,'Flow.h5'),xvel_label, ... 
            [IDXstart(1),IDXstart(2),IDXstart(3)], ...
            [IDXend(1)-IDXstart(1)+1,IDXend(2)-IDXstart(2)+1,IDXend(3)-IDXstart(3)+1]));
        vy = single(wraphdf5reader(fullfile(directory,'Flow.h5'),yvel_label, ... 
            [IDXstart(1),IDXstart(2),IDXstart(3)], ...
            [IDXend(1)-IDXstart(1)+1,IDXend(2)-IDXstart(2)+1,IDXend(3)-IDXstart(3)+1]));
        vz = single(wraphdf5reader(fullfile(directory,'Flow.h5'),zvel_label, ... 
            [IDXstart(1),IDXstart(2),IDXstart(3)], ...
            [IDXend(1)-IDXstart(1)+1,IDXend(2)-IDXstart(2)+1,IDXend(3)-IDXstart(3)+1]));
        ...
        % use for flow python
        % Load x,y,z components of velocity (cropped) - single frame
         vx = wraphdf5reader(fullfile(directory,'Flow.h5'),'/VX', ... 
         ...
         vy = wraphdf5reader(fullfile(directory,'Flow.h5'),'/VY', ... 
             [IDXstart(1),IDXstart(2),IDXstart(3),j], ...
             [IDXend(1)-IDXstart(1)+1,IDXend(2)-IDXstart(2)+1,IDXend(3)-IDXstart(3)+1,1])*10;
         vz = wraphdf5reader(fullfile(directory,'Flow.h5'),'/VZ', ... 
             [IDXstart(1),IDXstart(2),IDXstart(3),j], ...
             [IDXend(1)-IDXstart(1)+1,IDXend(2)-IDXstart(2)+1,IDXend(3)-IDXstart(3)+1,1])*10;
